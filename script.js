...
function initializeApp() {
    ...
    quizzes.forEach(function(quiz) {
        ...
        quizPage.innerHTML = '<button class="back-btn">← Back</button><div class="header"><h2> TOPIC SENTENCE:</h2><h1> <br>' + quiz.description + '</h1><div class="score">Score: <span class="score-current">0</span> / <span class="score-total">' + quiz.questions.length + '</span></div></div><div class="cards-container"><div class="cards-grid"></div></div><div class="result-screen"><h2>🎉 Quiz Complete!</h2><p>Your Final Score: <span class="final-score"></span></p><button class="btn btn-restart">Try Again</button><button class="btn btn-menu">Back to Menu</button></div>';
...
function updateQuizCardsStatus() {
    quizzes.forEach(function(quiz) {
        ...
        if (quizCard && state.answeredCards.length === quiz.questions.length) {
